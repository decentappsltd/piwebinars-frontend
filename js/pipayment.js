...
async function auth() {
  try {
    ...
    function onIncompletePaymentFound(payment) {
      ...
      axios.post(`${url}payment/cancel`, function (res, req) {
        res.send(data);
      });
    }
    Pi.authenticate(scopes, onIncompletePaymentFound)
      .then(async function (auth) {
        ...
        axios
          .post(`${url}register`, function (req, res) {
...
function buyWebinar(webinarId, creatorId, categoryId) {
  if (status == "true") {
    const creditAmountJSON = axios.get("${url}profile");
    ...
    if (creditAvailable > 1) {
      axios.post(`${url}payments/credit`, async function (req, res) {
        ...
        Pi.createPayment(
          ...
          },
          {
            onReadyForServerApproval: function (paymentId) {
              axios.post(`${url}payment/approve`, function (res, req) {
                res.send(paymentId);
              });
            },
            onReadyForServerCompletion: function (paymentId, txid) {
              const user_id = creatorId;
              axios.post(`${url}payment/complete`, function (res, req) {
                res.send(user_id, paymentId, txid);
              });
              showWebinar(webinarId, creatorId, categoryId);
              const profileCurrentUser2 = axios.get("${url}profile");
              ...
              axios.post(
                `${url}post/purchases/${userId1}/${categoryId}/${creatorId}/${webinarId}`
              );
            },
...
const showWebinar = (webinarId, creatorId, categoryId) => {
  ...
  const purchasedWebinarJSON = axios.get(
    `${url}upload/${collection_name}/${userId}/${file_id}`
  );
  ...
  document.getElementById("followCreator").onclick = function () {
    axios.post(`${url}auth_follow_unfollow/${userId}`);
  };
};
...
function buyCredits(creditAmount) {
  axios.post(`${url}profile/credit`, function (res, req) {
